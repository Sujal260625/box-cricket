...
interface LoginPageProps {
  onLogin: (email: string, password: string, userData?: any) => Promise<void>;
  onPhoneLogin: (phone: string, otp: string) => Promise<void>;
...
export function LoginPage({ onLogin, onPhoneLogin, onSocialLogin, navigateTo }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      if (isPhoneLogin && otpSent && otp) {
        await onPhoneLogin(phone, otp);
      } else if (!isPhoneLogin && otpSent) {
        handleVerifyOtp(e);
      } else if (showPasswordInput) {
        await onLogin(email, password, currentUserDetails);
      } else {
        await handleEmailNext(e);
      }
    } catch (err: any) {
